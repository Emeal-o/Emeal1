...
with tabs[3]:
    ...
    if not done:
        ...
        with col_race:
            ...
            try:
                ff1r_results = ff1r.results if ff1r else None
            except: ff1r_results = None
            if ff1r_results is not None and len(ff1r_results)>0:
                ...
                """, unsafe_allow_html=True)
                for _,row in ff1r_results.iterrows():
                ...
                # DNFs
                dnfs=ff1r_results[~ff1r_results["Status"].isin(["Finished","+1 Lap","+2 Laps","+3 Laps",""])]
        ...
        with col_q:
            ...
            try:
                ff1q_results = ff1q.results if ff1q else None
            except: ff1q_results = None
            if ff1q_results is not None and len(ff1q_results)>0:
                st.markdown("""
                <div class="tw">
                ...
                """, unsafe_allow_html=True)
                for _,row in ff1q_results.iterrows():
                    pos=str(row.get("Position","?"))
                    fn=str(row.get("FirstName",""))[:1]; ln=str(row.get("LastName","?"))
                    team=row.get("TeamName",""); col=tc(team)
                    abbr=row.get("Abbreviation","???")
...
with tabs[4]:
    ...
    if not done:
        ...
        try:
            ff1l_laps = ff1l.laps if ff1l else None
        except: ff1l_laps = None
        if ff1l_laps is not None and len(ff1l_laps)>0:
            ldf=ff1l_laps.copy().dropna(subset=["LapTime"])
